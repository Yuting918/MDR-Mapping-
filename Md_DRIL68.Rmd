...
library(readxl)
allric <- read_excel("DRIL68_ric.xlsx", sheet = "ALL")
GLSric <- allric[allric$TraitName == "GLS_lsm",]
GWric <- allric[allric$TraitName == "DRIL68_2EN_GW",]
...
### Thresholds
library(readxl)
NC344oH7Btic <- read_excel("tic_for_all.xlsx", sheet= "DRIL68")
GW.tic <- subset(NC344oH7Btic, TRAIT=="19GW_2EN_NOM65")$TIC
GLS.tic <- subset(NC344oH7Btic, TRAIT=="GLS_NOM65")$TIC
NCLB.tic <- subset(NC344oH7Btic, TRAIT=="NLB_NOM65")$TIC
SCLB.tic <- subset(NC344oH7Btic, TRAIT=="SLB_NOM65")$TIC
```
...
rrcovHD.out <- merge(map, rrcovHD.out, by.x = "MarkerName",by.y = "MarkerID") #189 obs
...
dim(forplot[which(apply(forplot[,c("gls.tic","nlb.tic","slb.tic","gw.tic")]==TRUE,1,any)),]) #17 29
dim(subset(forplot, rrcovHD.FDR<0.01)) #36 29

# the minimum distance of the adjusted p-value which is less than 0.01
Md.1pFDR_threshold <- min(subset(rrcovHD.out, rrcovHD.FDR<0.01)$rrcovHD.distance) #[1] 4.082515
...
p2 <- ggplot(mnhtn, aes(x=cMcum, y=rrcovHD.distance, shape=Disease, fill=Disease)) +
    ...
    scale_y_continuous(limits=c(0,35), expand=c(0,0), breaks=seq(0,35,5)) +     # remove space between plot area and x axis
