...
library(readxl)
allric <- read_excel("DRIL38_ric.xlsx", sheet = "ALL")
GLSric <- allric[allric$TraitName == "GLS_lsm",]
GW_19_ric <- allric[allric$TraitName == "DRIL38_19_GW",]
NCLBric <- allric[allric$TraitName == "NLB_lsm",]
SCLBric <- allric[allric$TraitName == "SLB_lsm",]

GLSric <- subset(GLSric, Environment=="Mean") #210 obs
GW_19_ric <- subset(GW_19_ric, Environment=="Mean")
...
lods <- cbind.data.frame(MarkerID=GLSric$MarkerName,
                         gls.lod=GLSric$LOD,
                         gw_19.lod=GW_19_ric$LOD,
                         nclb.lod=NCLBric$LOD,
                         sclb.lod=SCLBric$LOD,
                         gls.add=GLSric$Add,
                         gw_19.add=GW_19_ric$Add,
...
### Thresholds
library(readxl)
Ki3Oh7Btic <- read_excel("tic_for_all.xlsx", sheet= "DRIL38")
GW_19.tic <- subset(Ki3Oh7Btic, TRAIT=="19_GW")$TIC
GLS.tic <- subset(Ki3Oh7Btic, TRAIT=="GLS")$TIC
NCLB.tic <- subset(Ki3Oh7Btic, TRAIT=="NLB")$TIC
SCLB.tic <- subset(Ki3Oh7Btic, TRAIT=="SLB")$TIC
```
...
lods.rrcovHD$gw_19.prob <- pchisq(lods.rrcovHD$gw_19.lod*(2*log(10)),df=1,lower.tail=FALSE)/2
...
lods.rrcovHD$gw_19.neglog10p <- -log10(lods.rrcovHD$gw_19.prob)
...
rrcovHD.out <- merge(map, rrcovHD.out, by.x = "MarkerName",by.y = "MarkerID") #205 obs
rrcovHD.out <- rrcovHD.out[order(rrcovHD.out$Chr, rrcovHD.out$cM),]
rrcovHD.out[which(rrcovHD.out$rrcovHD.FDR<0.01),]


rrcovHD.out$gw_19.tic <- rrcovHD.out$gw_19.lod >= GW_19.tic
...
rrcovHD.out[intersect(which(apply(rrcovHD.out[,c("gw_19.tic","gls.tic","nlb.tic","slb.tic")]==TRUE,1,any)), which(rrcovHD.out$rrcovHD.FDR<0.01)),]
...
## Decalre significance if LOD > tic for at least 1 disease QTL and Md FDR of 1%
forplot$rrcovHD.FDR.sig[intersect(which(apply(forplot[,c("gw_19.tic","gls.tic","nlb.tic","slb.tic")]==TRUE,1,any)), which(forplot$rrcovHD.FDR<0.01))] <- "sig."

# Retrieve or set the dimension of an object
dim(forplot[which(apply(forplot[,c("gls.tic","nlb.tic","slb.tic","gw_19.tic")]==TRUE,1,any)),]) #20 29
dim(subset(forplot, rrcovHD.FDR<0.01)) #44 29

# the minimum distance of the adjusted p-value which is less than 0.01
Md.1pFDR_threshold <- min(subset(rrcovHD.out, rrcovHD.FDR<0.01)$rrcovHD.distance) #[1] 3.847387
...
levels(mnhtn$disease) <- c("GW","GLS","NCLB","SCLB", "GW & NCLB", "GW & GLS & NCLB","MO")
...
mnhtn[which(mnhtn$gls.lod<GLS.tic & mnhtn$nclb.lod<NCLB.tic & mnhtn$sclb.lod<SCLB.tic & mnhtn$gw_19.lod<GW_19.tic),]$disease <- "MO"
mnhtn[which(mnhtn$gw_19.lod>GW_19.tic & mnhtn$nclb.lod<NCLB.tic),]$disease <- "GW"
mnhtn[which(mnhtn$sclb.lod>SCLB.tic),]$disease <- "SCLB"
mnhtn[which(mnhtn$nclb.lod>NCLB.tic & mnhtn$gw_19.lod<GW_19.tic),]$disease <- "NCLB"
mnhtn[which(mnhtn$gw_19.lod<GW_19.tic & mnhtn$gls.lod>GLS.tic & mnhtn$nclb.lod<NCLB.tic),]$disease <- "GLS"
mnhtn[which(mnhtn$gw_19.lod>GW_19.tic & mnhtn$nclb.lod>NCLB.tic & mnhtn$gls.lod<GLS.tic),]$disease <- "GW & NCLB"
mnhtn[which(mnhtn$gw_19.lod>GW_19.tic & mnhtn$nclb.lod>NCLB.tic & mnhtn$gls.lod>GLS.tic),]$disease <- "GW & GLS & NCLB"
...
p2 <- ggplot(mnhtn, aes(x=cMcum, y=rrcovHD.distance, shape=Disease, fill=Disease)) +
    ...
    scale_shape_manual(values = c(24,24,24,24,23,22,1)) +
    scale_fill_manual(values=c("#D55E00","#009E73" , "#56B4E9","#0072B2",'#E7B800','#E7B800','gray97')) +
    ...
    scale_y_continuous(limits=c(0,45), expand=c(0,0), breaks=seq(0,45,5)) +     # remove space between plot area and x axis
